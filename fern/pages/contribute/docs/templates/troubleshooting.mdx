...
```json
{
  "contexts": [
    {
      "app": { "browser": "chrome", "executablePath": "/custom/path/to/chrome" }
    }
  ]
}
```
...
```json
{ "contexts": [{ "app": { "browser": "chrome", "headless": true } }] }
```
...
2. **Try a minimal test**: Rule out test-specific issues `json { "tests": [{ "steps": [{ "action": "goTo", "url": "https://example.com" }] }] } `
