...
jobs:
  doc-detective:
    ...
    steps:
      ...
      - restore_cache:
          keys:
            - dd-deps-{{ checksum "package-lock.json" }}
      - run:
      ...
      - save_cache:
          paths:
            - node_modules
          key: dd-deps-{{ checksum "package-lock.json" }}
      - run:
          name: Run tests
...
Use the `--exit-on-fail` flag or check exit codes to fail the pipeline when tests fail:
